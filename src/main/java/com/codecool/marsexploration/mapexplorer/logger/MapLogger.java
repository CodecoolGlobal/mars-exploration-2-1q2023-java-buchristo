...
public class MapLogger {
    ...
    public void drawExploredMap(Map map, Set<Coordinate> visitedCoordinates, java.util.Map<String, Set<Coordinate>> scannedRessources, Coordinate shipLandingCoordinate) {
        visitedCoordinates
                .forEach(c -> map.getRepresentation()[c.X()][c.Y()] = "\u001B[35m.\u001B[0m");
        scannedRessources.get("%")
                .forEach(mineral -> map.getRepresentation()[mineral.X()][mineral.Y()] = "\u001B[32m%\u001B[0m");
        scannedRessources
                .get("*").forEach(water -> map.getRepresentation()[water.X()][water.Y()] = "\u001B[32m*\u001B[0m");
